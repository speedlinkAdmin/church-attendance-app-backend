...
# def import_hierarchy_from_excel(file_path, state_name, state_code="RIV-CEN"):
#     """
#     Imports hierarchical data (State -> Region -> OldGroup -> Group -> District) from Excel.
#     file_path: path to the Excel file
#     state_name: name of the state (e.g. "Rivers Central")
#     """
...
#         print(f"Row {i}: {[str(cell) if pd.notna(cell) else 'NaN' for cell in df.iloc[i]]}")

#     # Create the state (assuming one state per import)
#     state = State.query.filter_by(name=state_name).first()
#     if not state:
#         state = State(name=state_name, code=state_code)
#         db.session.add(state)
...
#         # FIX: Safely convert to string before calling .strip()
#         # Detect OldGroup
#         if pd.notna(row[0]) and "OLD GROUP" in str(row[0]).upper():
#             old_group_name = str(row[0]).strip()  # FIX: Convert to string first
#             current_old_group = OldGroup.query.filter_by(
#                 name=old_group_name, state_id=state.id
#             ).first()
#             if not current_old_group:
...
#                     code=old_group_name[:4].upper(),
#                     state_id=state.id,
#                     region_id=1  # default region placeholder if unknown
#                 )
#                 db.session.add(current_old_group)
#                 db.session.commit()
#             continue
...
#         # Detect Group row
#         if pd.notna(row[1]):
#             group_name = str(row[1]).strip()  # FIX: Convert to string first
#             current_group = Group.query.filter_by(
#                 name=group_name, old_group_id=current_old_group.id
#             ).first()
#             if not current_group:
...
#                     code=group_name[:4].upper(),
#                     state_id=state.id,
#                     region_id=1,  # default placeholder
#                     old_group_id=current_old_group.id
#                 )
#                 db.session.add(current_group)
#                 db.session.commit()
...
#         # Detect District row
#         if pd.notna(row[3]):
#             district_name = str(row[3]).strip()  # FIX: Convert to string first
#             district_code = str(row[0]).strip() if pd.notna(row[0]) else district_name[:4].upper()  # FIX: Also convert row[0] to string
#             existing = District.query.filter_by(
#                 name=district_name,
#                 group_id=current_group.id,
#                 old_group_id=current_old_group.id,
...
#             district_code = row[0] if pd.notna(row[0]) else district_name[:4].upper()
#             existing = District.query.filter_by(
#                 name=district_name,
#                 group_id=current_group.id,
#                 old_group_id=current_old_group.id,
